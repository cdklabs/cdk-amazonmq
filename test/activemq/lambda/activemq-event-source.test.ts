...
describe("ActiveMqEventSource", () => {
  test.each([
    ...
    const credentials = Secret.fromSecretNameV2(
      stack,
      "TestSecret",
      "TestSecret",
    );
    ...
    let props: { [key: string]: any } = {
      EventSourceArn: {
        "Fn::GetAtt": ["TestBrokerBD4D5330", "Arn"],
      },
      FunctionName: {
  ...
  test.each([
    ...
    expect(() =>
      ...
      `Maximum batch size must be between 1 and 10000 inclusive (given ${batchSize})`,
    );
  });
});
