...
describe("ActiveMqBrokerDeployment", () => {
  test.each([
    ...
    const broker: IActiveMqBrokerDeployment = new ActiveMqBrokerInstance(
      stack,
      "TestBroker",
      {
    ...
    expect(broker.metricEstablishedConnectionsCount(opts)).toBeInstanceOf(
      Metric,
    );
    expect(broker.metricHeapUsage(opts)).toBeInstanceOf(Metric);
    expect(
      broker.metricInactiveDurableTopicSubscribersCount(opts),
    ).toBeInstanceOf(Metric);
    expect(broker.metricJobSchedulerStorePercentUsage(opts)).toBeInstanceOf(
      Metric,
    );
    expect(broker.metricJournalFilesForFastRecovery(opts)).toBeInstanceOf(
      Metric,
    );
    expect(broker.metricJournalFilesForFullRecovery(opts)).toBeInstanceOf(
      Metric,
    );
    expect(broker.metricMqttMaximumConnections(opts)).toBeInstanceOf(Metric);
    expect(broker.metricNetworkConnectorConnectionCount(opts)).toBeInstanceOf(
      Metric,
    );
    ...
    expect(broker.metricOpenwireMaximumConnections(opts)).toBeInstanceOf(
      Metric,
    );
