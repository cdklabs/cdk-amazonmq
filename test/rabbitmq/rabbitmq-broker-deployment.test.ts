...
import { RabbitMqBrokerEngineVersion, RabbitMqBrokerInstance, IRabbitMqBrokerDeployment } from '../../src';
...
describe('RabbitMqBrokerDeployment', () => {
  ...
  test.each([
    ...
    expect(broker.metricMessageUnacknowledgedCount(opts)).toBeInstanceOf(Metric);
    ...
    expect(broker.metricRabbitMQIOWriteAverageTime(opts)).toBeInstanceOf(Metric);
  });
});